...
// ─── Auth Orphan Heal Email ───────────────────────────────────────────────────

function buildOrphanHealHtml(name, resetLink) {
  const firstName = name ? name.trim().split(/\s+/)[0] : null;
  const safeFirstName = firstName ? escapeHtml(firstName) : null;
  const greeting = safeFirstName ? `Hi ${safeFirstName},` : "Hi,";
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Finish setting up your Weeber account</title>
</head>
<body style="margin:0;padding:0;background:#F5F4F0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:#F5F4F0;padding:48px 20px 32px;">
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:540px;">
        ...
        <tr><td style="background:#FEFEFE;border:1px solid #E8E6E1;border-radius:3px;overflow:hidden;">
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
            ...
            <tr><td style="padding:40px 40px 36px;">
              ...
              <p style="margin:0 0 20px;font-size:15px;color:#4A4A4F;line-height:1.75;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
                It looks like you started signing up before but didn't finish. We've recovered your account and set up your workspace — use the link below to set a password and log in.
              </p>
            ...
            <tr><td style="padding:0 40px 36px;" align="center">
              <a href="${safeLink}" style="display:inline-block;background:#0B0B0C;color:#FEFEFE;text-decoration:none;font-size:15px;font-weight:700;padding:14px 36px;border-radius:2px;letter-spacing:-0.1px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
                Set your password →
              </a>
            </td></tr>
        ...
        <tr><td style="padding:24px 0 0;text-align:center;">
          <p style="margin:0;font-size:11px;color:#9A9AA0;line-height:1.6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
            You received this because an account was created for this email at weeber.ai
          </p>
        </td></tr>
...
function buildOrphanHealText(name, resetLink) {
  const firstName = name ? name.trim().split(/\s+/)[0] : null;
  const greeting = firstName ? `Hi ${firstName},` : "Hi,";

  return `${greeting}

It looks like you started signing up before but didn't finish. We've recovered your account and set up your workspace.

Use the link below to set a password and log in. This link expires in 24 hours.
...
async function sendOrphanHealEmail(email, name, resetLink) {
  if (!resend) {
    logger.warn("RESEND_API_KEY not configured — skipping orphan heal email");
    return;
  }
  ...
  const { data, error } = await resend.emails.send({
    from: env.RESEND_FROM_EMAIL,
    to: [email],
    subject: "Finish setting up your Weeber account",
    html: buildOrphanHealHtml(name, resetLink),
    text: buildOrphanHealText(name, resetLink),
    replyTo: "hello@weeber.ai",
    tags: [{ name: "category", value: "auth" }],
  });

  if (error) {
    logger.error({ err: error, to: email }, "Orphan heal email failed");
  } else {
    logger.info({ emailId: data?.id, to: email }, "Orphan heal email sent");
  }
}
...
module.exports = {
  ...
  resolveRecipients,
  buildBroadcastHtml,
  sendEnterpriseConfirmation,
  buildEnterpriseConfirmationHtml,
  sendOrphanHealEmail,
  escapeHtml,
};
