...
"Next phase" fixes from the UI/UX audit: the three-way design-token split is down to one system, semantic status colors go through tokens instead of raw Tailwind palette classes, the onboarding checklist stops overpromising, in-progress onboarding survives a refresh, and loading states are down to two consistent idioms.

- **Token consolidation (`src/index.css` + 20 files)**: deleted the legacy hex "surface/text" token set (`--color-bg`, `--color-text`, `--color-text-secondary`, `--color-text-placeholder`, `--color-border-subtle`) and migrated every consumer (`text-text-muted` → `text-muted-foreground`, `bg-surface` → `bg-card`, `bg-surface-2` → `bg-muted`, `text-text` → `text-foreground`, `border-text` → `border-foreground`, `bg-bg` → `bg-background`) onto the shadcn oklch tokens that already drive every `components/ui/` primitive. `--color-surface`/`--color-surface-2`/`--color-text-muted` are kept as a documented, minimal compatibility shim solely because `TwilioAccountSection.tsx` (Hands-Off Zone) still references them — that file was not touched. Also repointed 4 raw `var(--color-*)` references inside `index.css` itself (`.section-label`, `input::placeholder`, `.focus-ring`, `.custom-scrollbar`) that would otherwise have silently lost their dark-mode values.
- **Semantic color sweep (25 files)**: added dark-mode overrides for `--color-success/warning/danger/info` (previously fixed-value, no dark variant) and replaced raw status-color classes (`bg-emerald-500`, `text-red-600`, `text-amber-500`, `bg-blue-500`, etc.) with the matching token across success/error/warning/info dots, badges, banners, and form-validation messages. Left three deliberate exceptions where forcing 4 tokens onto a genuinely multi-way categorical palette would lose information: `Outcomes.tsx`'s 7-state call-outcome legend, `VoiceLibrary.tsx`'s voice-type tint badges, `VariablesPanel.tsx`'s variable-category colors, plus `BuyNumberDialog.tsx`'s capability tags and the marketing `Waitlist.tsx` page (separate `.marketing` design register). Also fixed a real bug found along the way: `Integrations.tsx` had `border-success\30` (a stray backslash) instead of `border-success/30`, silently no-op'ing the "connected" highlight.
- **`src/pages/Dashboard.tsx`**: the "Resume setup" button now only appears when `pick_vertical`/`create_agent`/`test_and_golive` — the three steps `OnboardingModal` actually walks through — still need doing, with a line clarifying that connecting tools, adding knowledge, and getting a number each have their own page instead. Previously it always appeared and silently couldn't finish 3 of the 6 checklist items.
- **`src/components/onboarding/OnboardingModal.tsx`**: in-progress onboarding (selected preset, business name, voice, step position) now persists to `localStorage` and restores on reopen/refresh, capped at the "voice" step so a restored draft never jumps straight into the agent-creation/test step and risks a duplicate agent. Cleared on successful completion. Also replaced the preset-grid and voice-grid loading spinners with skeleton grids shaped like the actual cards; the three remaining spinners (agent creation, initializing, in-button "Creating…") were kept since they represent an action in progress, not a content region with a known shape.
- **`src/apps/customer/CustomerApp.tsx`**: replaced the pulsing-logo route-level loader with a `Skeleton`-based placeholder. In practice this fallback only fires for the standalone auth pages (Login/Signup/...), since authenticated routes suspend at `AppShell`'s own inner `PageSkeleton` boundary first.

Verification: `npx tsc --noEmit` clean; `npm run lint` shows no new warnings; backend `npm test` 140/140 pass (frontend-only change, run per convention).
