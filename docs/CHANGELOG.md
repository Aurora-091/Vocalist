...
- **Webhook Path Parameter Resolution**: Updated URL template path parameter conversion to convert double-braced URL parameters (e.g., `{{CALL_ID}}`) to single braces (e.g., `{CALL_ID}`) as expected by ElevenLabs. Dynamically generates a matching `path_params_schema` object mapping these parameters to the conversation's dynamic variables rather than using LLM description guesses.
