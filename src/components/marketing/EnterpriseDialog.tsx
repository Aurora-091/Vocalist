...
const STEPS: Step[] = [
  ...
  {
    key: "callVolume",
    sectionLabel: "Question 2 of 5",
    question: "How many customer calls do you handle per month?",
    type: "select",
    options: CALL_VOLUMES,
  },
  {
    key: "painPoint",
    sectionLabel: "Question 3 of 5",
    question: "What's the biggest issue you're trying to solve?",
    type: "select",
    options: PAIN_POINTS,
  },
  {
    key: "timeline",
    sectionLabel: "Question 4 of 5",
...
export function EnterpriseDialog({ open, onOpenChange }: Props) {
  ...
  function handleClose(v: boolean) {
    if (!v) {
      // Reset on close after animation completes
      setTimeout(() => {
        setStep(0);
        ...
        {submitted ? (
          <div className="px-8 pt-8 pb-8">
            ...
            <p className="mt-4 text-sm text-[var(--m-text-muted)]">
              ...
              <a href="mailto:enterprise@weeber.ai" className="text-[var(--m-text)] underline underline-offset-2">
                enterprise@weeber.ai
              </a>
            </p>
          </div>
        ) : (
          <div className="px-8 pt-6 pb-8">
            ...
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[11px] tracking-[.14em] uppercase text-[var(--m-text-secondary)]">
                  Enterprise inquiry
                </span>
              </div>
              {/* Progress bars */}
            ...
            {/* Section label */}
            <div className="font-mono text-[11px] tracking-[.14em] uppercase text-[var(--m-text-muted)] mb-3">
              {current.sectionLabel}
            </div>

            {/* Question */}
            <h3 className="font-display text-[22px] md:text-[26px] font-extrabold tracking-[-0.03em] leading-[1.1] text-[var(--m-text)] mb-6">
              {current.question}
            </h3>
            ...
            {current.optional && (
              <p className="text-xs text-[var(--m-text-muted)] mb-6">
                This is optional — hit Continue to skip
              </p>
            )}
            ...
            <div className="flex items-center gap-3 mt-8">
              {step > 0 && (
                <button
                  ...
                  className="px-4 py-2.5 text-sm font-medium text-[var(--m-text-secondary)] hover:text-[var(--m-text)] transition-colors"
                >
                  Back
                </button>
              )}
