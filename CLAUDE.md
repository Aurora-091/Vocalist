...
- **Voice provider abstraction**: `src/providers/voice/interface.js` is the base class; `factory.js` registers only `elevenlabs` + `mock` (vapi/retell are compiled but unregistered, reserved for a later swap). Telephony has a parallel abstraction in `src/providers/telephony/` (twilio, plivo, exotel, vobiz — the latter two for Indian telephony).
